...
<style>
  ...
  * { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Inter', sans-serif; }
  .stApp { background: linear-gradient(135deg, #F3F4F6 0%, #E5E7EB 100%); }
  .main .block-container { padding: 2rem 3rem; max-width: 960px; }
  ...
  .app-header h1 { color: white !important; font-weight: 700; font-size: 1.8rem; margin: 0; }
  .app-header p { color: rgba(255,255,255,0.85); margin: 0.5rem 0 0 0; font-size: 0.95rem; }
  ...
  .voucher-card-body { padding: 1.5rem; }
  .voucher-title {
    font-size: 1.1rem;
    font-weight: 600;
    color: #1E3A8A;
  ...
  .debit-row { color: #DC2626; font-weight: 500; }
  .credit-row { color: #2563EB; font-weight: 500; }
  .amount-cell { font-weight: 700; font-family: 'SF Mono', 'Cascadia Code', 'Courier New', monospace; font-size: 1.15rem; }
  ...
  .high-risk { border-left-color: #DC2626; background: #FEF2F2; border-color: #FECACA; }
  .medium-risk { border-left-color: #F59E0B; }
  .low-risk { border-left-color: #3B82F6; }
  ...
  .audit-badge-safe { background: #D1FAE5; color: #065F46; }
  .audit-badge-low { background: #DBEAFE; color: #1E40AF; }
  .audit-badge-medium { background: #FEF3C7; color: #92400E; }
  .audit-badge-high { background: #FEE2E2; color: #991B1B; }
...
st.markdown(
  ...
  '<h1>智能会计凭证推荐系统</h1>'
  '<p>输入业务描述，AI 自动匹配 130+ 会计场景并智能审计</p>'
  '</div>',
  unsafe_allow_html=True,
)
...
user_input = st.text_area(
  ...
  placeholder="请输入业务描述（例如：购买办公桌椅2000元；支付第三季度房租）",
  height=100,
...
def display_voucher_result(result, user_input: str, verdict_result: dict = None):
  ...
  st.markdown(
    ...
    f'<div class="voucher-card-header">推荐会计分录</div>'
    '<div class="voucher-card-body">',
    unsafe_allow_html=True,
  )

  if is_ai:
    st.warning("[AI] 此为 AI 推荐结果，请人工复核确认后再做账！")
  ...
  if all_findings:
    ...
    for f in all_findings:
      sev_class = {"high": "high-risk", "medium": "medium-risk", "low": "low-risk"}.get(f["severity"], "")
      sev_icon = {"high": "[高]", "medium": "[中]", "low": "[低]"}.get(f["severity"], "")
      st.markdown(
        f'<div class="warning-box {sev_class}">'
        f'{sev_icon} <strong>{f["type"]}</strong><br>'
        f'<span style="color:#374151;">{f["message"]}</span><br>'
        f'<small style="color:#6B7280;">{f["suggestion"]}</small>'
        f"</div>",
        unsafe_allow_html=True,
      )
  ...
  if all_tax:
    ...
    for t in all_tax:
      ...
      st.markdown(
        ...
        f'<span style="color:#374151;">{t["message"]}</span>'
        f"</div>",
        unsafe_allow_html=True,
      )
...
def _run_matching(text: str, verdict_result: dict):
  ...
  # ── 第二层：AI 大模型兜底匹配 ──
  with st.spinner("[AI] 规则库未匹配到，正在调用 AI 智能分析..."):
...
if st.session_state.get("recommend_triggered", False):
  ...
  if not text:
    ...
    if len(_core) <= 2:
      st.warning(
        "**未识别到核心财税词汇，请补充关键业务词（如采购、维修、工资等）。**"
      )
    ...
    if verdict_result.get("verdict") == "fuzzy_match":
      fuzzy_acct = verdict_result.get("fuzzy_account", "")
      st.warning(f"**此为模糊匹配结果，请人工核实科目。** 系统推荐科目：`{fuzzy_acct}`")
