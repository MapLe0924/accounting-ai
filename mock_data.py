...
def find_scenario(user_input: str):
    ...
    keyword_map = [
        ...
        (["自产 福利", "自产产品 福利", "自产商品 福利", "发给员工", "发放给员工", "产品发福利", "自产电脑发福利"], 30),  # id=31，需同时出现"自产"+"福利"
        # ── 所有者权益类 ──
        ...
        # ── 三期补漏 ──
        (["通讯费", "电话费", "网络费", "宽带费", "话费"], 98),  # id=99
        (["加油费", "停车费", "过路费", "车辆费", "油费", "私车公用"], 99),  # id=100
        (["保险费", "财产保险", "综合险", "企业财产险"], 100),  # id=101
        (["印刷费", "名片", "宣传册", "印刷品", "打印费"], 101),  # id=102
        (["劳保用品", "劳动保护", "安全帽", "劳保", "防护用品"], 102),  # id=103
        (["低值易耗品", "一次摊销", "低耗摊销", "低值易耗"], 103),  # id=104
        (["修缮", "围墙", "大门维修", "房屋修缮"], 104),  # id=105
        (["领用材料", "领用原材料", "车间领用", "生产领用", "生产领料"], 105),  # id=106
        (["存货跌价", "存货跌价准备", "存货减值", "计提存货跌价"], 106),  # id=107
        (["周转材料报废", "包装物报废", "包装箱报废"], 107),  # id=108
        (["五五摊销", "领用低值"], 108),  # id=109
        (["支付教育经费", "职工教育经费", "培训费支出", "教育经费支出"], 109),  # id=110
        (["支付工会经费", "上缴工会经费", "工会经费支付"], 110),  # id=111
        (["辞退", "裁员", "经济补偿", "辞退福利", "裁员补偿", "解聘"], 111),  # id=112
        (["预收转收入", "预收确认收入", "预收账款转收入", "预收款交货"], 112),  # id=113
        (["委托代销", "委托代销", "代销发出", "发出商品"], 113),  # id=114
        (["预缴所得税", "预缴企业所得税", "季度预缴"], 114),  # id=115
        (["留抵退税", "留抵退税", "增值税留抵", "退留抵税", "收到留抵"], 115),  # id=116
        (["递延所得税资产", "递延所得税", "确认递延"], 116),  # id=117
        (["融资租赁", "融资租入", "融资租入", "融资租赁设备"], 117),  # id=118
        (["研发资本化", "研发支出资本化", "开发支出转入", "资本化转入"], 118),  # id=119
        (["出售无形资产", "出售专利", "出售非专利", "变卖无形资产", "卖专利"], 119),  # id=120
        (["长期股权投资", "购入股权", "购买股权", "长期股权投资"], 120),  # id=121
        (["坏账收回", "已核销又收回", "核销后收回", "坏账又收回"], 121),  # id=122
        (["国债", "债券投资", "购入国债", "持有至到期", "债权投资"], 122),  # id=123
        (["汇兑损益", "汇兑损失", "汇率变动", "外币借款汇兑", "汇兑收益"], 123),  # id=124
        (["结转销售费用", "销售费用结转", "结转销售费用"], 124),  # id=125
        (["结转财务费用", "财务费用结转", "结转财务费用"], 125),  # id=126
        (["结转所得税费用", "所得税费用结转", "结转所得税"], 126),  # id=127
        (["盈余公积补亏", "盈余公积弥补", "补亏", "弥补亏损"], 127),  # id=128
        (["股票股利", "股票股利", "送股", "转作股本", "转增股本"], 128),  # id=129
        (["前期差错", "以前年度损益调整", "前期差错", "少计折旧", "上年差错", "差错更正"], 129),  # id=130
        # ── 四期复杂场景 ──
        (["购入设备运费", "设备运输费", "设备普票", "运费普票", "价款税运费", "购入设备价款"], 130),  # id=131
        (["计提工资分部门", "工资生产车间行政销售", "生产工人车间管理行政销售", "工资分部门"], 131),  # id=132
        (["销售折让", "折让", "红字发票折让", "给予折让", "质量折让"], 132),  # id=133
        (["预付保险费", "预付全年保险", "保险费摊销", "预付保险摊销"], 133),  # id=134
        (["短期借款按月", "借款按月计提", "借款年利率", "按月付息"], 134),  # id=135
        (["已提跌价", "跌价准备销售", "存货跌价出售", "跌价商品销售"], 135),  # id=136
        (["信用减值损失", "坏账新准则", "信用减值计提", "计提坏账核销"], 136),  # id=137
        (["提取法定任意", "法定任意盈余", "盈余公积法定任意", "提取宣告股利", "净利润提取法定任意"], 137),  # id=138
        (["样品赠送", "样品赠送给客户", "作为样品赠送", "赠送样品", "自产样品"], 138),  # id=139
    ]
